 ...
 * Menggunakan TensorFlow.js + Jimp (pure JS) untuk kompatibilitas Windows tanpa native bindings
 */

// Gunakan Jimp untuk pemrosesan gambar (resize, pixel access) tanpa modul native
...
class ServerTensorFlowPredictor {
    ...
    async predictFromImagePath(imagePath) {
        ...
        try {
            ...
            // Load image menggunakan Jimp dan resize ke 128x128 - sama dengan frontend
            const image = await Jimp.read(imagePath);
            image.resize(128, 128, Jimp.RESIZE_BILINEAR);

            // Ambil pixel data RGBA dan konversi ke Tensor [128,128,3]
            const { data, width, height } = image.bitmap; // data adalah Buffer RGBA
