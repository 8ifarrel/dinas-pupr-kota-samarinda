...
- **Cek Status** — warga melihat status pesanan berdasarkan nomor telepon yang dipakai saat mendaftar, lengkap dengan filter status/bulan/tahun dan halaman detail per pesanan
- **Kelola Pesanan (Admin)** — daftar pesanan (dengan filter status/bulan/tahun), ubah status, cetak pesanan
...
| Bagian | Isi |
|---|---|
| Model | `App\Models\Silalad` (tabel `silalad`), `App\Models\SilaladApiKey` (tabel `silalad_api_keys`) |
...
Sudah diuji lewat migrasi, seeding, dan permintaan HTTP nyata di lingkungan terisolasi sebelum branch ini didorong: login admin, pengisian formulir pendaftaran publik, ubah status pesanan (termasuk ke `Dibatalkan`), serta seluruh endpoint API — mencakup permintaan tanpa kunci, dengan kunci milik fitur lain, dengan data tidak lengkap, dengan pasangan kecamatan–kelurahan yang tidak cocok, dan jalur sukses untuk wilayah Samarinda maupun luar Samarinda. Seluruh data uji dihapus kembali setelahnya.
