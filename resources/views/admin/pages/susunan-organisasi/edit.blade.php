...
@section('slot')
  ...
  <form action="{{ route('admin.susunan-organisasi.update', $susunan->id_susunan_organisasi) }}" method="POST">
    @csrf
    {{-- Gunakan POST jika route hanya menerima POST --}}
    @method('PUT')
    ...
    <div class="mb-4">
      ...
      @else
        ...
        <select name="kelompok_susunan_organisasi" id="kelompok_susunan_organisasi" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required>
    ...
    <div class="mb-1">
      ...
      <input type="text" name="nama_susunan_organisasi" id="nama_susunan_organisasi" value="{{ $susunan->nama_susunan_organisasi }}"
        ...
        @foreach ($parentOptions ?? [] as $item)
          @if (
            (in_array($item->kelompok_susunan_organisasi, ['Bidang', 'UPTD']) &&
                (empty($item->id_susunan_organisasi_parent) || $item->id_susunan_organisasi_parent == 0 || $item->id_susunan_organisasi_parent == 1)) ||
    ...
    <div class="mb-4" id="fungsional_parent_group" style="display: none;">
      ...
      <select name="fungsional_parent" id="fungsional_parent"
        class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
        <option value="" disabled {{ !$susunan->is_jabatan_fungsional ? 'selected' : '' }}>-- Pilih Susunan Organisasi Induk --</option>
        @foreach ($parentOptions ?? [] as $item)
          @if (
            (in_array($item->kelompok_susunan_organisasi, ['Bidang', 'UPTD']) &&
                (empty($item->id_susunan_organisasi_parent) || $item->id_susunan_organisasi_parent == 0 || $item->id_susunan_organisasi_parent == 1)) ||
    ...
    <div class="mb-4">
      ...
      <input id="tupoksi_susunan_organisasi" type="hidden" name="tupoksi_susunan_organisasi" value="{{ $susunan->tupoksi_susunan_organisasi }}">
