        ...
        <tbody>
          ...
          @foreach ($susunan_organisasi as $item)
            @if (empty($item->id_susunan_organisasi_parent) ||
              ...
              <tr @if ($item->children && count($item->children) > 0) data-children="{{ json_encode($item->children) }}" @endif>
                <td class="{{ $item->children && count($item->children) > 0 ? 'dt-control' : '' }}">
                </td>
                <td>{{ $rowNumber++ }}</td>
              ...
              <x-admin.modal-hapus :id="$item->id_susunan_organisasi" :aksi="route('admin.struktur-organisasi.susunan-organisasi.destroy', $item->id_susunan_organisasi)">
