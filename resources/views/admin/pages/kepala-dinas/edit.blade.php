    ...
    <div class="mb-4">
      ...
      <select id="golongan_pegawai" name="golongan_pegawai"
        ...
        @foreach($golonganList as $gol)
          <option value="{{ $gol }}" @if($kepalaDinas->golongan == $gol) selected @endif>{{ $gol }}</option>
        @endforeach
      </select>
    </div>
    <div class="mb-4">
      ...
      <div class="relative group foto-viewer-wrapper h-28 sm:h-32">
        <label
          ...
          tabindex="0">
          <div class="foto-placeholder flex flex-col items-center justify-center pt-5 pb-6 {{ $kepalaDinas->foto ? 'hidden' : '' }}">
      ...
      if (cropConfirmBtn) cropConfirmBtn.addEventListener('click', function() {
        if (cropper) {
          cropper.getCroppedCanvas().toBlob(function(blob) {
            const croppedFile = new File([blob], lastFile ? lastFile.name : 'cropped_foto.jpg', {type: blob.type});
