{{--
  Kerangka halaman daftar API Key, dipakai bersama oleh provider
  "akun-kelurahan" dan "hantu-banyu" (Jalan Peduli sengaja punya
  berkasnya sendiri, tidak ikut memakai kerangka ini).
  ...
  - $page_title/$page_description  teks judul halaman (dari layout admin).

  Dokumentasi API-nya berbeda per provider (endpoint dan field yang
  diterima memang berbeda), jadi bagian itu di-include dari partial
  masing-masing: admin.pages.super-admin.api-key.{provider}.dokumentasi.
--}}
...
@section('document.body')
  <div class="border shadow bg-white rounded-lg p-5 mb-8 space-y-3">
    ...
    @include('admin.pages.super-admin.api-key.' . $provider . '.dokumentasi')
  </div>

  <a href="{{ route('admin.super.api-key.' . $provider . '.create') }}"
...
@section('document.end')
  <script>
    ...
    function viewApiKey(id) {
      $.ajax({
        url: `/api/${apiKeyProvider}-keys/${id}`,
        type: 'GET',
        headers: {
    ...
    function confirmRegenerate() {
      $.ajax({
        url: `/api/${apiKeyProvider}-keys/${currentApiKeyId}/regenerate`,
        type: 'POST',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
    ...
    function confirmDelete() {
      $.ajax({
        url: `/api/${apiKeyProvider}-keys/${currentApiKeyId}`,
        type: 'DELETE',
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
