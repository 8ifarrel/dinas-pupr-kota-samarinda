...
@section('css')
  ...
  <style>
    .slider-viewer-wrapper { height: 220px; }
    .slider-placeholder { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; }
    .slider-preview { background: #fff; }
  </style>
@endsection
  ...
  <form action="{{ route('admin.slider.update', $slider->id_slider) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-4 flex gap-2">
      <div class="w-4/5">
        ...
        <input type="text" name="judul_slider" id="judul_slider" value="{{ $slider->judul_slider }}"
      ...
      <div class="w-1/5">
        ...
        <select name="is_visible" id="is_visible" class="mt-1 block w-full p-2 border border-gray-300 rounded-md"
          required>
          <option value="1" {{ $slider->is_visible ? 'selected' : '' }}>Ditampilkan</option>
          <option value="0" {{ !$slider->is_visible ? 'selected' : '' }}>Disembunyikan</option>
        </select>
      </div>
    </div>
    ...
    <div class="mb-4">
      ...
      <div class="relative group slider-viewer-wrapper h-28 sm:h-32">
        <label
          ...
          tabindex="0">
          <div class="slider-placeholder flex flex-col items-center justify-center pt-5 pb-6 {{ $slider->foto_slider ? 'hidden' : '' }}">
          ...
          <img id="slider-preview"
            class="absolute inset-0 w-full h-full object-contain rounded-lg bg-white aspect-[16/9] {{ $slider->foto_slider ? '' : 'hidden' }}"
            src="{{ $slider->foto_slider ? Storage::url($slider->foto_slider) : '' }}" />
          <input name="foto_slider" id="foto_slider" type="file" accept="image/*"
            class="hidden slider-input" />
        </label>
        <button type="button"
          class="w-[30px] h-[30px] bg-white rounded-full text-red-500 hover:text-red-700 shadow-lg border border-black flex items-center justify-center absolute top-2 right-2 z-10 remove-slider-btn {{ $slider->foto_slider ? '' : 'hidden' }}"
        ...
        <a type="button" id="edit-slider-button"
          class="p-3.5 w-3 h-3 bg-white rounded-full text-black shadow-lg border border-black flex items-center justify-center absolute bottom-2 right-2 z-10 {{ $slider->foto_slider ? '' : 'hidden' }}">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function pushSliderHistory(src) {
        if (sliderHistoryPointer < sliderHistory.length - 1) sliderHistory = sliderHistory.slice(0, sliderHistoryPointer + 1);
        sliderHistory.push(src); sliderHistoryPointer = sliderHistory.length - 1; updateRevertSliderBtn();
      }
      ...
      if (input) input.addEventListener('change', function() {
        if (input.files && input.files[0]) {
          ...
          reader.onload = function(ev) {
            ...
            cropperSlider = new Cropper(imageToCropSlider, {viewMode: 1, autoCropArea: 1, aspectRatio: 2.368 / 1});
          };
      ...
      if (cropSliderConfirmBtn) cropSliderConfirmBtn.addEventListener('click', function() {
        if (cropperSlider) {
          cropperSlider.getCroppedCanvas().toBlob(function(blob) {
            ...
            const reader = new FileReader();
            reader.onload = function(ev) { setSliderPreviewAndHistory(ev.target.result); };
            reader.readAsDataURL(croppedFile);
            cropperSlider.destroy(); cropperSlider = null; cropperModalSlider.classList.add('hidden');
