...
@section('document.body')
  <a href="{{ route('admin.agenda-kegiatan.create') }}"
  ...
  <div class="w-full p-4 rounded-lg shadow-xl sm:p-8 mt-5">
    <div class="relative overflow-x-auto text-sm md:text-base">
      <table id="agenda" class="stripe hover row-border table-auto" style="width:100%">
        <thead>
          <tr>
            <th>#</th>
            <th>Nama</th>
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-4 md:p-5 space-y-4">
          <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
            Apakah Anda yakin ingin menghapus agenda <strong id="deleteModalNama"></strong>?
          </p>
        </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      var table = $('#agenda').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.agenda-kegiatan.datatable') }}",
        order: [[2, 'desc']],
        columnDefs: [
      ...
      $('#agenda tbody').on('click', '.btn-delete-agenda', function() {
        var id = $(this).data('id');
        var nama = $(this).data('nama');
        var url = "{{ route('admin.agenda-kegiatan.destroy', ':id') }}".replace(':id', id);
        $('#deleteModalForm').attr('action', url);
        $('#deleteModalNama').text(nama);
