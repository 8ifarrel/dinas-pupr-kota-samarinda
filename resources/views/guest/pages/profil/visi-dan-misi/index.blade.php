        ...
        <p class="mb-4">Untuk mewujudkan Visi Walikota Samarinda, Dinas Pekerjaan Umum Dan Penataan Ruang Kota Samarinda melaksanakan misi sebagai berikut:</p>
				...
				@foreach ($misi as $item)
					<p class="m-2 sm:m-5 flex items-center">
						<span class="border bg-brand-blue rounded-lg w-9 h-9 sm:text-xl font-bold text-white sm:w-11 sm:h-11 flex items-center justify-center me-2 shrink-0">{{ $loop->iteration }}</span> {{ $item->deskripsi_misi }}
					</p>
				@endforeach				
      @endif
    </div>
