  ...
  <div class="max-w-5xl mx-auto bg-white rounded-2xl shadow p-8">
    ...
    <form method="GET" action="{{ route('guest.sedot-tinja.status') }}" class="mb-8 flex gap-3">
      <input type="text" name="nomor_telepon_pelanggan" value="{{ request('nomor_telepon_pelanggan') }}" 
             placeholder="Masukkan Nomor Telepon Anda (kosongkan untuk lihat semua)"
          ...
          <tbody>
            @foreach($result as $item)
              <tr class="odd:bg-white even:bg-gray-50">
                <td class="px-4 py-3 font-semibold text-blue-700">#{{ $item->id }}</td>
                <td class="px-4 py-3">{{ $item->created_at->format('d M Y') }}</td>
                <td class="px-4 py-3">{{ $item->nama_pelanggan }}</td>
                <td class="px-4 py-3">{{ $item->nomor_telepon_pelanggan }}</td>
    ...
    @else
      <p class="text-center text-gray-600">Tidak ada data ditemukan.</p>
    @endif
